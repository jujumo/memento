...
Takes two 3-D vectors and returns a single 3-D vector.
The resulting 3-D vector is perpendicular to both inputs.
...
If two vectors are aligned or if either one has zero length, then their cross product is zero.
