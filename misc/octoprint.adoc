...
.navigationbar
----
Done: <span style='display: inline-block;width:24px;'>[progress]%</span> ,
Left: <span style='display: inline-block;width:54px;'>[printtime_left]</span> ,
Due: <span style='display: inline-block;width:34px;'>[estimated_end_time]</span>
----

.printer display
----
[progress]% TL : [printtime_left] ETA : [estimated_end_time]
----

.browser title:
----
[printer_state] : [progress]% TL: [printtime_left] ETA :[estimated_end_time]
----

- MQTT:

setup: "Host", "Base topic" name, "username" and "password".

- HomeAssistant Discovery :

setup: "Device name"  and "Model"
...
Let's keep in mind the evolution of the camera stack:
https://github.com/cp2004/OctoPrint-CameraStreamer-Control

== reverse proxy

My first choice would be caddy.

=== caddy

==== install caddy

Check whether Caddy is already available:
...
=== haproxy :
haproxy would be my second choice:
...
use link:go2rtc.adoc[go2rtc] for video stream

In octoprint `settings` > `Classic webcam` :

 - Stream URL: `/webcam/api/stream.mjpeg?src=webcam`
 - Snapshot URL: `http://127.0.0.1/webcam/api/frame.jpeg?src=webcam`
