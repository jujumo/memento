...
[cols="<2m,<5",options="header",frame=none,grid=rows]
|=====================================
...
[cols="2,2,8",frame=none,grid=rows]
|===
...
# first pass
ffmpeg -i movie.mp4                                                                       \
    -vf vidstabdetect=stepsize=6:shakiness=10:accuracy=15:result=motions.trf              \
    -f null -
# second pass
ffmpeg -i movie.mp4                                                                       \
    -vf vidstabtransform=input=motions.trf:smoothing=60:relative=1,unsharp=5:5:0.8:3:3:0.4 \
...
| `-i movie.mp4`                            | input video
| `vidstabdetect=...:result=motions.trf`    | detects motion and stores tracking data
| `-f null -`                               | discards output during analysis because only the motion file is needed
| `vidstabtransform=input=motions.trf`      | uses the saved motion data to stabilize the video
| `smoothing=60`                            | smooths camera movement more aggressively
| `relative=1`                              | applies stabilization relative to the detected motion
| `unsharp=...`                             | restores some perceived sharpness after stabilization
| `-vcodec libx264`                         | re-encodes video with H.264
| `-preset veryslow -tune film`             | favors compression efficiency and film-friendly encoding
| `-acodec copy`                            | copies audio without re-encoding
| `-y smooth.mp4`                           | overwrites the output file if it already exists
|=====================================
...
- Background sequence: `bg_%04d.png` (no alpha)
- Foreground sequence: `fg_%04d.png` (with alpha channel)

The basic command is:
[source,bash]
----
ffmpeg -i bg_%04d.png -i fg_%04d.png -filter_complex "[0][1] overlay" -c:v libx264 -pix_fmt yuv420p output.mp4
----
...
| `-i bg_%04d.png`                   .2+| Input image sequences.
| `-i fg_%04d.png`
| `-filter_complex "[0][1] overlay"`    | Overlays the second input (with alpha) over the first.
| `-c:v libx264`                        | Encodes the output as H.264 (common for MP4).
| `-pix_fmt yuv420p`                    | Ensures compatibility with most players.
|=====================================

== stereo 3D

source: https://trac.ffmpeg.org/wiki/Stereoscopic[https://trac.ffmpeg.org/wiki/Stereoscopic]
...
----
 ...
 - qscale: to setup the audio quality. 0 is better.
...
NOTE: ffmpeg fails to properly write spherical metadata tag. You will need `exiftool`.

[source,bash]
----
exiftool -XMP-GSpherical:Spherical="true" -XMP-GSpherical:ProjectionType="equirectangular" input.mp4
----
